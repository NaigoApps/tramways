import {RelevantPoint, RoadMap, RoadMapContent} from "../../../api/generated";

export default class SmartMap {

    private changeUuid: number;
    private onChanged: (map: RoadMapContent) => void;
    private map: RoadMapContent;
    private elements: Map<string, RelevantPoint>;

    constructor(map: RoadMapContent, onChanged: (map: RoadMapContent) => void) {
        this.changeUuid = 0;
        this.onChanged = onChanged;
        this.map = map;
        this.elements = new Map<string, RelevantPoint>();

        map.points.forEach(point => {
            this.elements.set(point.id, point);
